...
async def test_save_file_os_error():
    ...
    with (
        patch("npo.modules.images.services.check_max_upload_size"),
        patch("npo.modules.images.services.check_required_space"),
        patch("npo.modules.images.services.open", side_effect=OSError("Disk full")) as mock_open,
    ):
        with pytest.raises(StorageError) as exc_info:
            await save_file(mock_upload_file, mock_image)
...
async def test_save_file_domain_error_cleanup():
    ...
    with (
        patch("npo.modules.images.services.check_max_upload_size") as mock_check_size,
        patch("npo.modules.images.services.check_required_space"),
        patch("npo.modules.images.services.open", MagicMock()),
        patch("npo.modules.images.services.clean_upload_file") as mock_clean_file,
    ):
...
async def test_compute_pixel_hash_failure():
    ...
    with (
        patch("npo.modules.images.services.is_web_format", return_value=True),
        ...
        with pytest.raises(ImageDecodingError) as exc_info:
            await compute_pixel_hash(mock_image)
...
async def test_compute_pixel_hash_sync():
    ...
    with (
        patch("npo.modules.images.services.is_web_format", return_value=False),
        ...
        pixel_hash = _compute_pixel_hash_sync(mock_image, mock_preview_bytes)
...
async def test_compute_perceptual_hash():
    ...
    with (
        ...
        with pytest.raises(ImageProcessingError) as exc_info:
            await compute_perceptual_hash(mock_image)
